...
void AliTRDmcmSim::Print(Option_t* const option) const
{
    ...
    if (opt.Contains("T")) {
        ...
        for (Int_t iTrkl = 0; iTrkl < fTrackletArray->GetEntriesFast(); iTrkl++) {
            ...
            printf("tracklet %i: x= %f, y=%f z=%f  yb=%d, zb=%d\n", iTrkl, 
                    ((AliTRDtrackletMCM*) (*fTrackletArray)[iTrkl])->GetX(),
                    ((AliTRDtrackletMCM*) (*fTrackletArray)[iTrkl])->GetY(),
                    ((AliTRDtrackletMCM*) (*fTrackletArray)[iTrkl])->GetZ(),
                    ((AliTRDtrackletMCM*) (*fTrackletArray)[iTrkl])->GetYbin(),
                    ((AliTRDtrackletMCM*) (*fTrackletArray)[iTrkl])->GetZbin(),
                    ((AliTRDtrackletMCM*) (*fTrackletArray)[iTrkl])->GetSlope(),
                    ((AliTRDtrackletMCM*) (*fTrackletArray)[iTrkl])->GetOffset());
    ...
    for (Int_t cpu = 0; cpu < 4; cpu++) {
        ...
        else
        {
            ...
            else
            {
                ...
                std::cout << "Tracklet " << fTrackletArray->GetEntries() << " slope : " << fitSlope << " offset : " << fitOffset << std::endl;
...
void AliTRDmcmSim::Tracklet()
{
    ...
    if (fNHits == 0)
        return;
    TrackletSelection();
    //print out the eadc values.
    //
    FitTracklet();
    std::string filename("trap_raw");
    filename += std::to_string(fDetector) + "_" + std::to_string(fRobPos)+ "_" + std::to_string(fMcmPos);
    std::string rootfilename(filename+".root");
    std::ofstream outfile("trapsimoutput.txt",   std::ios_base::app );
    outfile << "new trap sim ............................. " << std::endl;
    Text(outfile);
    Printout(outfile);
   // outfile << "Now for cf dat :"<< std::endl;
   // Cfdat(outfile);
  //  Printout(outfile);
    outfile << "Now for raw :"<< std::endl;
    Raw(outfile);
    Printout(outfile);
    outfile << "Now for the tracklets" << std::endl;
    Print("f");//PrintTrackletsXml(outfile);

    outfile << "end of trap sim .......................... " << std::endl;
...
ostream& AliTRDmcmSim::Printout(ostream& os)
{
    // output implementation
os << "in ostream of trdmcmsim" << std::endl;
        ...
        for (Int_t iTimeBin = 0; iTimeBin < fNTimeBin; iTimeBin++) {
            for (Int_t iChannel = 0; iChannel < AliTRDfeeParam::GetNadcMcm(); iChannel++) {
                os << std::setw(5) << 10
                    ...
                    << std::setw(5) << (fADCF[iChannel][iTimeBin])
    ...
    // ----- raw data ouptut -----
        ...
        Int_t bufLength   = ProduceRawStream(&buf[0], bufSize);
